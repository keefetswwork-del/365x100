...
export function BooksView({ client, online }: BooksViewProps) {
  ...
  if (document) {
    ...
    return <section role="tabpanel" className="mt-7">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,0.36fr)_minmax(0,0.64fr)]">
        <aside className="space-y-5 rounded-[1.5rem] border border-[var(--line)] bg-white/55 p-5 lg:sticky lg:top-4 lg:self-start">
          <label className="block text-sm font-bold">Cover title<input value={draftTitle} maxLength={120} onChange={(event) => { setDraftTitle(event.target.value); setDraftDirty(true); }} className="mt-2 min-h-11 w-full rounded-xl border border-[var(--line)] bg-white px-3 font-serif text-xl outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]" /></label>
          <label className="block text-sm font-bold">Cover photograph<select value={document.publication.coverMediaId ?? ""} disabled={working} onChange={(event) => void changeCover(event.target.value || null)} className="mt-2 min-h-11 w-full rounded-xl border border-[var(--line)] bg-white px-3 outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] disabled:opacity-50"><option value="">Typographic cover</option>{coverOptions.map((option) => <option key={option.id} value={option.id}>{option.date}</option>)}</select></label>
          {document.publication.mode === "ai" && <div><p className="text-sm font-bold">Editable editorial sections</p><label className="mt-3 block text-xs font-bold">Month in review<textarea value={draftEditorial.review} onChange={(event) => { setDraftEditorial((current) => ({ ...current, review: event.target.value })); setDraftDirty(true); }} rows={7} className="mt-1 w-full rounded-xl border border-[var(--line)] bg-white p-3 text-sm leading-6 outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]" /></label><label className="mt-3 block text-xs font-bold">Themes<input value={draftEditorial.themes.join(", ")} onChange={(event) => { setDraftEditorial((current) => ({ ...current, themes: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })); setDraftDirty(true); }} className="mt-1 min-h-11 w-full rounded-xl border border-[var(--line)] bg-white px-3 outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]" /></label><div className="mt-3 flex flex-wrap gap-2">{(["title", "review", "themes", "moments", "quotations"] as const).map((section) => <button key={section} type="button" disabled={working} onClick={() => void regenerate(section)} className="rounded-full border border-[var(--line)] px-3 py-2 text-xs font-bold capitalize">Regenerate {section}</button>)}</div></div>}
          {document.publication.mode === "original" && (library.aiEntitled
            ? <button type="button" disabled={working} onClick={() => openAiDisclosure({ eligible: true, ended: true, entryCount: document.entries.length, monthEnd: document.publication.periodEnd, monthStart: document.publication.periodStart, publication: document.publication, words: model.totalWords, writingDays: model.writingDays })} className="min-h-11 rounded-full border border-[var(--ink)] px-4 text-sm font-bold">Create AI-edited version</button>
            : <a href="mailto:hello@365x100.com?subject=AI%20chapter%20beta%20access" className="inline-flex min-h-11 items-center rounded-full border border-[var(--line)] px-4 text-sm font-bold">Request AI beta access</a>)}
          <p className="text-xs font-semibold text-[var(--muted)]" role="status">{draftStatus}</p>
          <div className="flex flex-wrap gap-2"><button type="button" disabled={working || document.publication.state === "stale"} onClick={() => void approve()} className="min-h-11 rounded-full bg-[var(--accent)] px-5 text-sm font-bold text-white disabled:opacity-40">Approve chapter</button><PublicationPdfButton client={client} document={document} disabled={working || aiDownloadBlocked} model={model} /></div>
          {aiDownloadBlocked && <p className="text-xs leading-5 text-[var(--muted)]">Approve a current AI editorial version before downloading its PDF. Original-only PDFs can always use the latest journal entries.</p>}
          {document.publication.mode === "ai" && document.publication.state === "ready" && <fieldset className="border-t border-[var(--line)] pt-4"><legend className="text-xs font-bold">Factual accuracy check</legend><div className="mt-2 flex flex-wrap gap-2">{(["accurate", "needs-review", "invented-fact"] as PublicationFeedback[]).map((verdict) => <button type="button" key={verdict} onClick={() => void recordPublicationFeedback(client, document.publication.id, verdict)} className="rounded-full border border-[var(--line)] px-3 py-2 text-xs font-bold">{verdict.replaceAll("-", " ")}</button>)}</div></fieldset>}
          <button type="button" disabled={working} onClick={() => void removeGeneratedOutput()} className="text-sm font-bold text-red-900 underline underline-offset-4">Delete generated output</button>
        </aside>
  ...
  return <section role="tabpanel" className="mt-7">
    <div className="rounded-[1.5rem] bg-[var(--ink)] p-6 text-white"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--accent)]">Monthly chapters</p><h3 className="mt-2 font-serif text-4xl tracking-[-0.04em]">Turn a month of memories into a book.</h3><p className="mt-3 max-w-2xl text-sm leading-6 text-white/70">Ended months with at least 10 writing days can become a chronological original-entry chapter. AI editing is a private, founder-enabled beta.</p></div>
    {error && <p className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-900" role="alert">{error}</p>}
    <div className="mt-5 grid gap-3 sm:grid-cols-2">{library.items.map((month) => <article key={month.monthStart} className="rounded-2xl border border-[var(--line)] bg-white/55 p-5"><div className="flex items-start justify-between gap-3"><div><h4 className="font-serif text-2xl">{monthLabel(month.monthStart)}</h4><p className="mt-1 text-sm text-[var(--muted)]">{month.writingDays} writing days · {month.words.toLocaleString()} words</p></div><span className="rounded-full bg-[var(--sage)]/35 px-3 py-1 text-[0.68rem] font-bold uppercase tracking-wider">{stateLabel(month)}</span></div>{month.publication ? <button type="button" disabled={working} onClick={() => void openPublication(month.publication!.id)} className="mt-5 min-h-11 rounded-full bg-[var(--ink)] px-5 text-sm font-bold text-white">Open chapter</button> : month.eligible ? <div className="mt-5 flex flex-wrap gap-2"><button type="button" disabled={working} onClick={() => void createOriginal(month)} className="min-h-11 rounded-full bg-[var(--ink)] px-4 text-sm font-bold text-white">Create original chapter</button>{library.aiEntitled ? <button type="button" disabled={working} onClick={() => openAiDisclosure(month)} className="min-h-11 rounded-full border border-[var(--ink)] px-4 text-sm font-bold">Create AI-edited chapter</button> : <a href="mailto:hello@365x100.com?subject=AI%20chapter%20beta%20access" className="inline-flex min-h-11 items-center rounded-full border border-[var(--line)] px-4 text-sm font-bold">Request AI beta access</a>}</div> : <p className="mt-4 text-sm leading-6 text-[var(--muted)]">{month.ended ? "This month remains in your journal but did not reach 10 writing days." : `${Math.max(0, 10 - month.writingDays)} more writing days will qualify this month.`}</p>}</article>)}</div>
